...
export async function addRoom(prevState, formData) {
  ...
  try {
    ...
    if (existingName) {
      errors.roomName = "This room is already have";
      return { errors, message: "This room is already have"  };
    }
...
export async function updateRoom(roomId, prevState, formData) {
  ...
  try {
    ...
    return { error: "Failed to update room due to a server error", message: "Failed to update room due to a server error", };
  }
}
