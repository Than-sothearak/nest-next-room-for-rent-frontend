...
export default async function Home() {
  ...
  if (!session) {
    ...
    return (
      ...
      <div className="min-h-screen bg-white flex flex-col justify-between items-center px-6">
        <div
          className={`text-center transition-all duration-700 ease-out transform h-full w-full `}
        >


          <h1 className="text-4xl font-bold text-gray-800 mb-4 h-full w-full">Welcome to MyApp</h1>
          <p className="text-gray-500 text-lg mb-8 h-full w-full">
            Your smart way to manage everything.
          </p>

          <Link
            href="/login"
            className="bg-blue-600 text-white px-8 py-3 rounded-full text-lg font-semibold shadow-lg hover:bg-blue-700 active:scale-95 transition transform duration-300"
          >
            Get Started
          </Link>
        </div>
