// app/api/payway/checkout/route.js
...
export async function POST(req) {
  try {
    ...
    const params = {
      ...
      return_url: body.return_url ?? "https://your-site.com/payment-success",
      cancel_url: body.cancel_url ?? "https://your-site.com/payment-cancel",
    ...
    console.log("PayWay response:", response.json());
    if (!response.ok) {
      console.error("PayWay error:", result);
      return NextResponse.json({ error: result }, { status: 400 });
    }

    return NextResponse.json({ status: 200, tran_id });
  } catch (err) {
