...
export default async function Home() {
  ...
  if (!session) {
    ...
    return (
      ...
      <div className="w-full min-h-screen bg-white flex flex-col items-center justify-between px-6">
        <div
          className={`w-full text-center transition-all duration-700 ease-out transform flex flex-col min-h-screen  justify-around `}
        >

          <h1 className="text-4xl font-bold text-gray-800 mb-4">Welcome to Logment</h1>
          ...
          <Link
            href="/login"
            className="w-full bg-blue-600 text-white w-full px-8 py-3 rounded-full text-lg font-semibold shadow-lg hover:bg-blue-700 active:scale-95 transition transform duration-300"
          >
            Get Started
          </Link>
        </div>
