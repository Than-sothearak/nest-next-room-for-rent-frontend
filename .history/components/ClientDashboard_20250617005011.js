      ...
      <div className="mt-4 w-full justify-start items-start flex gap-4 p-4 bg-primary rounded-lg ">
        <div className="flex flex-col gap-2 w-full">
          <div className='flex justify-between w-full gap-4 '>
            ...
            <h1 className='w-full text-end'>DueDate: {formatDate(booking.dueDate)}</h1>
          </div>
          <div className='flex justify-between items-center w-full'>
            ...
            <div><h1 className='font-bold text-3xl text-end'>{booking.rent}$</h1><p className={`${booking.paymentStatus === "unpaid" ? 'text-red-500': 'text-green-500'} flex items-center gap-1`}><CgUnavailable />{booking.paymentStatus}</p></div>
          </div>
          <p className="text-green-500 text-xs">
            12% <span className="text-primarytext">more than previus week</span>
          </p>
