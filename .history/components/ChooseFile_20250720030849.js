         ...
         {files.length > 0 && (
          <div className="overflow-y-scroll flex flex-wrap justify-start gap-4 border-dashed border-secondary mt-2 rounded-md border-2 p-4 w-full">
            {files.map((fileData, index) => (
              <div
                key={index}
                className="w-full max-sm:w-full rounded-md relative group border"
              >
                {fileData.file.type.startsWith("image/") ? (
                  <div className="w-full border flex flex-col gap-2 border-secondary rounded-sm max-sm:w-full max-sm:h-52  ">
                   <div className="flex  w-10 h-10">
                     <Image
                      fill
                      src={fileData.preview}
                      alt="Preview"
                      className="p-2 rounsded-md  object-cover transition-opacity duration-300 group-hover:opacity-10"
                    />
                   </div>
