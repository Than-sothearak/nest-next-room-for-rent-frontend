      ...
      >
       ...
       {files.length > 0 && (
           ...
           {files.map((fileData, index) => (
             <div
               key={index}
               className="max-sm:w-full max-sm:h-52 rounded-md relative group border"
             >
               {fileData.file.type.startsWith("image/") ? (
                <div className="w-40 h-full border border-secondary rounded-sm max-sm:w-full max-sm:h-full ">
                   <Image
                   fill
                 ...
                 <p className="text-xs p-8">{fileData.file.name}</p>
               )}
               <button
                 type="button"
