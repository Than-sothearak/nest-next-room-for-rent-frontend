...
export default function BookingForm({
  ...
  const [manuallyChangedEndDate, setManuallyChangedEndDate] = useState(false);
...
useEffect(() => {
  if (startDate && contract && !manuallyChangedEndDate) {
    const start = new Date(startDate);
    const monthsToAdd = parseInt(contract);

    const end = new Date(start);
    end.setMonth(start.getMonth() + monthsToAdd);
    end.setDate(end.getDate() - 1); // adjust for full months
...
}, [startDate, contract, manuallyChangedEndDate]);

useEffect(() => {
  if (contract) {
    setManuallyChangedEndDate(false); // allow auto update again
  }
}, [contract]);
  return (
    <form
...
>
  ...
  {Array.from({ length: 12 }, (_, i) => (
    ...
    setManuallyChangedEndDate(true);
  }}
            required
            className="border rounded p-2"
          />
