  ...
  return (
    <form action={action} className="relative group inline-block w-full">
      ...
      {state?.error && showTooltip && (
        <div className={`z-20 absolute right-0 bg-red-100 text-red-800 border border-red-400 px-3 py-2 rounded shadow-lg w-full h-full text-sm mt-2  ${state.error ? "opacity-100" : "opacity-0"}`}>
        {state.error}
        </div>
      )}
