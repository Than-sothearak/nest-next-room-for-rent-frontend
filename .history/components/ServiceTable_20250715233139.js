...
const ServiceTable = ({
  ...
  return (
    <div className="mt-8">
      <div className="flex justify-start bg-slate-200  rounded-3xl w-max relative">
        {buttonName.map((item) => (
          <div key={item.name} className="">
            ...
            >
              <p className={`${item.name === isClick ? "bg-blue-500 text-primary" : ""
                } rounded-3xl p-4 z-0 transition-all duration-300 w-full `}>{item.name}</p>
              {item.name === isClick && (
                <div className="absolute bg-red-200 p-4 rounded-3xl w-full"></div>
              )}
