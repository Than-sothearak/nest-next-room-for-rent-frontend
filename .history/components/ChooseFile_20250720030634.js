            ...
            {files.map((fileData, index) => (
              ...
              >
                {fileData.file.type.startsWith("image/") ? (
                  <div className="w-full border border-secondary rounded-sm max-sm:w-full max-sm:h-52  ">
                   <div className="w-10 h-10">
                     <Image
                      fill
                      src={fileData.preview}
                      alt="Preview"
                      className="p-2 w-10 h-10 rounsded-md  object-cover transition-opacity duration-300 group-hover:opacity-10"
                    />
                   </div>
                    <p>file name</p>
                  </div>
