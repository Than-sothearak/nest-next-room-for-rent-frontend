...
const ClientService = ({
  ...
  return (
        ...
        {types.map((item, index) => {
          ...
          return (
            <div
              key={item.name}
              type="button"
              className={`relative border w-full justify-center items-center flex gap-4 p-4 ${item.color} text-primary rounded-lg `}
            >
              ...
              {isPending && (
                <form className="absolute top-1/2 left-1/2 w-full h-full -translate-x-1/2 -translate-y-1/2 bg-white flex items-center justify-center text-black px-2 py-1 rounded-md z-0">
                  ...
                  <p className="absolute top-1 left-2 text-xs">{formatDateTime(formatDate(services.filter(service => service.serviceType === item.service && service.status === 'pending')[0].startDate))}</p>
                  {services
              ...
              {isAccepted && (
                ...
                >
                  ...
                  <p className="absolute top-1 left-2 text-xs">{formatDateTime(formatDate(services.filter(service => service.serviceType === item.service && service.status === 'accepted')[0].startDate))}</p>
    ...
    <p className="text-lg font-bold">Accepted</p>
      ...
      {isClicked && (
        <div className="absolute z-30 top-1/2 left-1/2 overflow-y-auto p-4 bg-white rounded-lg max-lg:w-full w-1/2 -translate-x-1/2 -translate-y-1/2  shadow-xl">
          <ClientServiceForm
