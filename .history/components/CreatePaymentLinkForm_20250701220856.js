...
export default function CreatePaymentLinkForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {response  &&(
        <div className={`fixed translate-y-0 opacity-0 border-2 ${isClicked ? 'inset-10 opacity-40' : 'hidden'} bg-white top-1/4 left-1/2 -translate-x-1/2  transition-all duration-300 p-4 rounded-xl shadow-lg w-1/2 h-1/2 max-sm:w-full max-sm:top-2`}>
