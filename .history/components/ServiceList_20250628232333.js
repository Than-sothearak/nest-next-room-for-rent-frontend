...
const ServiceList = ({
  ...
  const [direction, setDirection] = useState("pending");

  const statuses = ["pending", "accepted", "canceled"];
  const [status, setStatus] = useState("pending")

const handleSort = (e, key) => {
   const currentIndex = statuses.indexOf(direction);
    const nextIndex = (currentIndex + 1) % statuses.length;
    setDirection(statuses[nextIndex]);
      ...
      <div className="text-xl font-bold flex gap-2 flex-wrap">
        {buttonName.map((item) => (
          <div key={item.name} className="relative mt-4">
            ...
            <Link
              href={`/dashboard/services?sortKey=${item.name.toLowerCase()}`}
              value={item.name}
