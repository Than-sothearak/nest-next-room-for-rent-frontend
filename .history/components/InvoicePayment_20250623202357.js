...
const InvoicePayment = ({ data }) => {
  ...
  const items = [
    {
      no: 1,
      type: `${getData?.roomId?.category} (${formatDate(getData.startDate)} - ${formatDate(getData.dueDate)})`,
      room: getData?.roomId?.roomName || '',  // Or get from populated room data
      level: getData?.roomId?.floor || '',
      price: Number(getData.roomId.price),
      qty: 1,
      off: 0,
      amt: Number(getData.roomId.price),
    },
    ...(Array.isArray(getData.bookingId?.properties)
  ? getData.bookingId?.properties.map((item, index) => ({
      no: index + 2,
      type: item?.service,
      room: '',
      level: '',
