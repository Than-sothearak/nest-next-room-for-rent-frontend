  ...
  return (
    <form action={action}>
      <button
        ...
        className={`${isPending ? 'cursor-not-allowed opacity-25 bg-green-200': ""} py-1 rounded-md lg:mx-4 lg:my-2 ${
          status === "paid"
            ? "text-green-600 "
            : "bg-green-200 text-green-600 hover:bg-green-500 hover:text-green-100"
        }`}
      >
       {isPending ? <p className="px-3">Pending...</p>:  status === "paid" ? <div className="flex items-center gap-1"><BiCheckCircle size={24}/> <p>Paid</p> </div>:  <p className="px-3">Pay</p>}
      </button>
    </form>
  );
};
