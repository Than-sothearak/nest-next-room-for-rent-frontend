...
import { FaEdit, FaShower, FaStickyNote } from "react-icons/fa";
import {
  FaBed,
  FaBook,
  FaEye,
...
import { MdBalcony, MdOutlineSmokeFree, MdOutlineTableBar } from "react-icons/md";
import { TbAirConditioning, TbDesk } from "react-icons/tb";
  ...
  return (
    <div className="my-4">
      <div className="w-full flex justify-between items-center my-4 gap-4">
        <h1 className="text-center text-2xl font-bold whitespace-nowrap">Room List</h1>
            ...
            <div className="w-full">
              <div className="relative w-full h-64 rounded-md group">
                ...
                <Image
                  ...
                  src={room?.imageUrls[0] || 'https://cdn.pixabay.com/photo/2016/11/30/08/48/bedroom-1872196_1280.jpg'}
                />
                <div className="p-4 absolute z-20 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center gap-4 text-white font-bold text-lg w-full flex-wrap">
                  ...
                  {room?.bookings && room.bookings?.length > 0 ? <Link
               ...
               transform  hover:scale-125 bg-black/70 p-4  rounded-full"
                  ...
                  <ButtonDelete session={session} id={room._id} setOptimisticData={setOptimisticData}/>
              ...
              <div className="w-full justify-between flex px-4 flex-wrap">
                <div
                  className={`flex gap-1 items-center ${room?.status === 1 ? "text-green-500" : "text-red-500"
                    }`}
                >
                  {room?.status === 1 ? (
