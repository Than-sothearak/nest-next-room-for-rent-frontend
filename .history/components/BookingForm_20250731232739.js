    ...
    <div>
      ...
      >
        ...
        <div className="space-y-4 bg-white shadow  p-4  rounded-xl">
          <h2 className="text-xl font-semibold">{booking ? "Edit Booking" : "Create Booking"} Invice: {String(booking?.invoiceId).padStart(5, '0')}</h2>
          ...
          <div className="grid gap-2">
            <label className="font-bold">Invoice send status  <span className="font-normal">(required)</span></label>
            <select
              name="paymentStatus"
              value={paymentStatus}
              onChange={(e) => setPayment(e.target.value)}
