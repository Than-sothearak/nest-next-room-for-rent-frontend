...
const ClientService = ({ services, isClicked, setIsClicked, booking, user }) => {
 ...
 const isPending = services?.some(
    (s) => s.serviceType === item.servic && s.status === "pending"
  );
          return (
            <div
            key={item.name}
      ...
      {isPending && (
        <div className="absolute top-1/2 left-1/2 w-full h-full -translate-x-1/2 -translate-y-1/2 bg-white/80 flex items-center justify-center text-black px-2 py-1 rounded-md z-20">
          <p className="animate-bounce p-2 rounded-md">Pending...</p>
          <button
            type="button"
            className="absolute bottom-2 bg-red-500 p-2 rounded-md right-2 text-primary hover:bg-red-700"
          >
            Cancel
          </button>
        </div>
