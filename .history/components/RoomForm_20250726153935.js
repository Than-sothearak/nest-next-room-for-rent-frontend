...
export default function RoomForm({
  ...
  const [formData, setFormData] = useState({
    ...
    status: room?.status === 0 ? room?.status : 1,// 1 = available, 0 = unavailable
            ...
            <div className="grid gap-2 grid-cols-2">
              ...
              {room?.imageUrls && room?.imageUrls.length > 0 ? (
                formData?.imageUrls.map((image, index) => (
                  <div
                    className={`${index === 0 ? "col-span-2" : ""
                      } overflow-hidden shadow-md relative z-0 aspect-square rounded-md bg-slate-500 group`}
                    key={index}
                  >
                    <Image
                      fill
               ...
               transform hover:scale-125 scale-100 bg-black opacity-50 hover:opacity-90 "
                ...
                <div
                  className={`col-span-2 rounded-md w-full min-h-32 bg-slate-500 aspect-square`}
                >
                  <img
                    alt={`Image`}
