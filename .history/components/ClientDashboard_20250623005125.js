...
export const ClientDashboard = async ({ session }) => {
  ...
  const payments = JSON.parse(JSON.stringify(await Payment.find({ userId: user._id }).populate('userId').populate('roomId').sort({ startDate: -1 })));
...
<InvoicePayment
  data={{
    ...
    roomId: {
      roomName: payment.roomId?.roomName || '',
      floor: payment.roomId?.floor || '',
      category: {
        category: payment.roomId?.category?.category || ''
      }
    },
    rent: payment.amount,
    properties: [], // add this if you support extra charges
  }}
/>
