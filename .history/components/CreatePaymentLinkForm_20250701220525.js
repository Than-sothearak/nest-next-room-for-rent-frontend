...
export default function CreatePaymentLinkForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {response  &&(
        <div className={`fixed ${isClicked ? 'inset-0 translate-y-10' : 'absolute w-0 h-0'} bg-white top-1/4 left-1/2 -translate-x-1/2  transition-all duration-300 p-4 rounded-xl shadow-lg w-1/2 h-1/2 max-sm:w-full max-sm:h-full max-sm:top-0`}>
