...
export default function CreatePaymentLinkForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {response &&  (
  <div
    className={`${isClicked ? 'opacity-100 ' : 'opacity-0'} fixed inset-0 z-50 bg-black/60 flex items-center justify-center`}
    onClick={() => setIsClicked(false)}
  >
    <div
      className="bg-white p-6 rounded-xl shadow-lg relative max-w-sm w-full"
