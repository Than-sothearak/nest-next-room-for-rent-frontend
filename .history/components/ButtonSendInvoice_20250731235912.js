...
const SendInvoiceButton = ({ bookingId, booking }) => {
 ...
 const router = useRouter();
  const [state, action] = useActionState(sendInvoiceById, undefined);
  console.log(state)
   useEffect(() => {
  ...
  return (
    <form action={action} className="relative group inline-block w-full">
      <button
            // ${isPending
            // ? "bg-gray-400 cursor-wait opacity-50"
            // : "bg-blue-600 text-white hover:bg-blue-300 hover:text-blue-900"} 
        className={`whitespace-nowrap z-0 px-3 w-full py-1 rounded-md relative bg-blue-600 text-white hover:bg-blue-300 hover:text-blue-900
      ...
      {state?.error && showTooltip && (
        <div className={`z-20 absolute right-0 bg-red-100 text-red-800 border border-red-400 px-3 py-2 rounded shadow-lg text-sm mt-2  ${state.error ? "opacity-100" : "opacity-0"}`}>
        {state.error}
        </div>
      )}
