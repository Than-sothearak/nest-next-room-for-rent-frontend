              ...
              {isPending && (
                <form className="absolute top-1/2 left-1/2 w-full h-full -translate-x-1/2 -translate-y-1/2 bg-white flex items-center justify-center text-black px-2 py-1 rounded-md z-0">
                  ...
                  <p className="animate-bounce p-2 rounded-md">Pending...</p>
                   <p className="absolute bottom-0 left-2">Schedule: {formatDate(services.filter(service => service.serviceType === item.service && service.status === 'pending')[0].startDate)}</p>
                  {services
