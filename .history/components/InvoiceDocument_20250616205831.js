...
const InvoicePDF = ({ data }) => {
  ...
  const items = [
    {
      no: 1,
      type: `Room Rent (${formatDate(getData.startDate)} - ${formatDate(getData.dueDate)})`,
      room: getData.roomId?.name || '',  // Or get from populated room data
      level: getData.roomId?.level || '',
  ...
  const deposit = Number(getData.deposit || 0);
    ...
    <Document>
      <Page style={styles.page} size="A4">
        ...
        <View style={styles.section}>
          <Text>To: Atn {getData.userId?.fullName || 'Tenant'}</Text>
