...
import { BiCheckCircle, BiDownload, BiPrinter } from 'react-icons/bi'
...
export const ClientDashboard = async ({ session }) => {
  ...
  const data = await Booking.findOne({ userId: user._id })
      ...
      <div className="mt-4 w-1/2 max-lg:w-full justify-start items-start flex gap-4 p-4 bg-primary rounded-lg ">
        <div className="flex flex-col w-full">
          <div className='flex justify-between w-full gap-4 '>
            ...
            <h1 className='text-end'>Due date: {formatDate(booking.dueDate)}</h1>
          </div>
          <div className='flex justify-between items-center w-full'>
            <div className='flex gap-2 flex-wrap w-full'>
              <div className='flex'><p>Id:</p> <p className='bg-slate-200 px-2 rounded-sm uppercase font-bold'> {booking.userId._id.slice(-10)}</p></div>
              <div className='flex'><p>Phone:</p> <p className='bg-slate-200 px-2 rounded-sm uppercase font-bold'>{booking.userId.phone}</p></div>
            </div>

            <div><h1 className='font-bold text-3xl text-end'>
              {booking.rent + booking.properties.reduce((sum, service)=> {
                return sum + Number(service.price);
              }, 0)
              
              }
              $</h1><p className={`${booking.paymentStatus === "unpaid" ? 'text-red-500' : 'text-green-500'} flex items-center gap-1`}>{booking.paymentStatus === "unpaid" ? <CgUnavailable /> : <BiCheckCircle />}{booking.paymentStatus}</p></div>
          </div>
          <div className='w-full mt-2'>
            {booking.userId?.telegramChatId ? <Link href={`https://t.me/wbc_notifications_bot?stop=${booking?.userId?._id}`} className='flex items-center gap-1 border w-max p-2 rounded-md bg-blue-500 text-white'>
              <BsTelegram /> <h1>Connected</h1>
            </Link> : <Link href={`https://t.me/wbc_notifications_bot?start=${booking?.userId?._id}`} className='flex items-center gap-1 border w-max p-2 rounded-md'>
        ...
        {payments.map(payment => (
          <div className='bg-slate-100 p-2 mt-2' key={payment._id}>
            <div className='flex items-center justify-between font-bold'>
              <p>Recipts: {formatDate(payment.startDate)}-{formatDate(payment.dueDate)} </p>
              <div className='flex items-center gap-4'>
           
                <p className='text-green-500'>{payment.status}</p>
                     <Link href={`invoice/${payment._id}`}><BiPrinter size={28}/></Link>
              </div>
            ...
            <div className='flex items-center justify-between'>
              <p>Paid at</p> <p>{formatDate(payment.paidAt)}</p>
            </div>
            ...
            <div className='flex items-center justify-between'>
              <p>Amount</p> <p>
    $ {payment.bookingId?.rent + payment.bookingId?.properties.reduce((sum, item) => {
