...
setLoading(false)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 relative">
      ...
      <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded w-1/2" disabled={loading}>
        {loading ? "Uploading..." : "Create Payment Link"}
      </button>

      {response && (
      <div className={`${isClicked ? 'opacity-100 -translate-y-1/2': 'opacity-0'} w-full absolute bg-white -top-96 left-1/2 -translate-x-1/2  transition-all duration-300 p-4 rounded-xl shadow-lg`}>
  ...
  <div className="absolute top-3 right-3">
        <button type='button' onClick={() => setIsClicked(false)}><CgClose /></button>
      </div>
        </div>
