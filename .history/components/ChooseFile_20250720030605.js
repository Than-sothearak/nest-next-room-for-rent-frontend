            ...
            {files.map((fileData, index) => (
              ...
              >
                {fileData.file.type.startsWith("image/") ? (
                  <div className=" border border-secondary rounded-sm max-sm:w-full max-sm:h-52  ">
                    <Image
                      fill
                      src={fileData.preview}
                      alt="Preview"
                      className="p-2 w-10 h-10 rounsded-md w-full h-full object-cover transition-opacity duration-300 group-hover:opacity-10"
                    />
                    <p>file name</p>
                  </div>
                ) : (
