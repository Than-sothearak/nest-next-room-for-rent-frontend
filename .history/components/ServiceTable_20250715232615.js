...
const ServiceTable = ({
  ...
  return (
    <div className="mt-4">
      <div className="flex justify-start bg-slate-200  rounded-3xl w-max">
        {buttonName.map((item) => (
          <div key={item.name} className="relative">
            ...
            >
              <p  className={`${item.name === isClick ? "bg-blue-500 text-primary" : ""
                } rounded-3xl p-4`}>{item.name}</p>
            </Link>
           {/* Highlight bubble behind active button */}
        {isClick && (
          <div
            className="absolute inset-0 bg-blue-500 rounded-3xl z-0 transition-all duration-300"
          /> 
        )}
