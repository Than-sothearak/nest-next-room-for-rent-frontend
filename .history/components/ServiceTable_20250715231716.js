...
const ServiceTable = ({
  ...
  return (
    <div className="mt-4">
      <div className="flex gap-4 bg-slate-100 w-full rounded-full">
        {buttonName.map((item) => (
          <div key={item.name} className="">
            ...
            <Link
              ...
              className={`${item.name === isClick ? "bg-transparent text-primary" : ""
                } rounded-xl`}
            >
              <p className="h-full bg-blue-500 p-2 rounded-lg">{item.name}</p>
            </Link>
          </div>
        ))}
      </div>
