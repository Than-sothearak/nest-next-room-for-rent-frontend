...
const AddPropertyForm = ({ formData, setFormData }) => {
  ...
  return (
    <div className="flex flex-col gap-2 mb-4 border p-4 rounded-md bg-slate-100">
      <button
        type="button"
        className={`${
      ...
      <div className="grid grid-cols-3 max-2xl:grid-cols-2 gap-4 max-lg:grid-cols-1">
        {formData?.properties.length > 0 &&
          formData.properties.map((part, index) => (
            <div
              key={index}
              className={`border border-blue-100 p-2 rounded-md max-lg:w-full text-sm relative ${
