  ...
  return (
    <form action={action}>
      <button
        ...
        className={`${isPending ? 'cursor-not-allowed opacity-25': ""} px-3 py-1 rounded-full lg:mx-4 lg:mt-2 font-semibold ${
          status === "paid"
            ? "bg-green-200 text-green-600 "
            : "bg-red-200 text-red-600 hover:bg-red-500 hover:text-red-100"
        }`}
      >
        {status === "paid" ? "Paid" : "Click if paid"}
      </button>
    </form>
  );
};
