...
export default function ChooseFile({files ,setFiles}) {
  ...
  return (
    <div className="">
      ...
      >
        <div className="w-full max-sm::h-96 h-full flex justify-center items-center gap-8 flex-col border-dashed border-secondary mt-2 rounded-md border-2 p-8">
       ...
       {files.length > 0 && (
           <div className="overflow-scroll flex flex-wrap justify-start gap-4 border-dashed border-secondary mt-2 rounded-md border-2 p-4 w-full">
           {files.map((fileData, index) => (
             <div
               key={index}
               className="max-sm:w-full rounded-md relative group border"
             >
               {fileData.file.type.startsWith("image/") ? (
                <div className="w-40 h-40 border border-secondary rounded-sm max-sm:w-full max-sm:h-52  ">
                   <Image
                   fill
