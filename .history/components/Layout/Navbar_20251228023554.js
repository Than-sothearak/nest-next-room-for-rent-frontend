...
export const Navbar = ({ session, user, navigation, servicesCount, link }) => {
  ...
  {pathArray.map((p, index) => {
    ...
    // Highlight if currentPathname starts with this path
    const isActive = currentPathname === linkPath || currentPathname.startsWith(linkPath + "/");
    
    console.log({ p, linkPath, isActive, currentPathname });
    return (
      <div key={index} className="flex items-center gap-1">
        <Link
          href={linkPath}
          className={`${
            isActive ? "font-semibold text-gray-900" : "hover:text-blue-500 transition"
          }`}
        >
          {p}
        </Link>
