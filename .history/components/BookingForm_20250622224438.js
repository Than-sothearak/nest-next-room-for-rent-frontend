  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {booking?.files.map((item, index) => (
          <div key={index} className="w-full flex gap-2 border p-2">
       
              <BsFillFileCodeFill /> {item}
          
            <button type="button"
              onClick={() => handleRemoveImage(index)} >Remove</button>
          </div>
        ))}
      </div>
