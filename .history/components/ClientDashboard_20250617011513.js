...
export const ClientDashboard = async ({ session }) => {
  ...
  const booking = JSON.parse(JSON.stringify(await Booking.findOne({ userId: user._id }).populate('userId').populate('roomId').populate({part: 'userId', populate: {part: 'payments'}})))
  console.log(booking)
  return (
    <div>
